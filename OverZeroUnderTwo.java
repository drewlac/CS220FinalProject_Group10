...
public class OverZeroUnderTwo {
	...
	public static void cpu1Gameplay3CPU(Hand cpuHand, Hand playerHand, Hand cpu2Hand, Hand cpu3Hand) {
		...
		if (gamePile.getTopCard().getValue() == 13 && playableCount > 0) {
			...
			// May not need this section to switch turns correctly. *** Possibly remove? ***
			if (gameFlow == 1) {
				cpu1Turn = false;
				playerTurn = true;
			} else if (gameFlow == 2) {
		...
		if (gamePile.getTopCard().getValue() == 10 && playableCount > 0 && cpu1Turn && gameFlow == 1) {
			cpu1Turn = false;
			cpu2Turn = false;
			cpu3Turn = true;
		...
		else if (gamePile.getTopCard().getValue() == 10 && playableCount > 0 && cpu1Turn && gameFlow == 2) {
			cpu1Turn = false;
			cpu3Turn = true;
			playerTurn = false;
			System.out.println("CPU 1 skipped your turn.");
	...
	private static void cpu2Gameplay3CPU(Hand cpuHand, Hand playerHand, Hand cpu1Hand, Hand cpu3Hand) {
		...
		// skip code for CLOCKWISE order
		if (gamePile.getTopCard().getValue() == 10 && playableCount > 0 && cpu2Turn && gameFlow == 1) {
			cpu2Turn = false;
			cpu3Turn = false;
			playerTurn = true;
		...
		// skip code for COUNTER-CLOCKWISE order
		else if (gamePile.getTopCard().getValue() == 10 && playableCount > 0 && cpu2Turn && gameFlow == 2) {
			cpu2Turn = false;
			cpu1Turn = false;
			playerTurn = true;
	...
	private static void cpu3Gameplay3CPU(Hand cpuHand, Hand playerHand, Hand cpu1Hand, Hand cpu2Hand) {
		...
		// skip code for CLOCKWISE order
		if (gamePile.getTopCard().getValue() == 10 && playableCount > 0 && cpu3Turn && gameFlow == 1) {
			cpu3Turn = false;
			playerTurn = false;
			cpu1Turn = true;
		...
		// skip code for COUNTER-CLOCKWISE order
		else if (gamePile.getTopCard().getValue() == 10 && playableCount > 0 && cpu3Turn && gameFlow == 2) {
			cpu3Turn = false;
			cpu2Turn = false;
			cpu1Turn = true;
